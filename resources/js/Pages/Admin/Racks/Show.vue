...
const openOfficeViewer = (file, viewerType) => {
    const fileUrl = encodeURIComponent(
        window.location.origin + route("files.download", file.id)
    );
...
<template>
    ...
    <AdminLayout>
        ...
        >
            <div
                class="animate-spin rounded-full h-8 w-8 border-b-2 border-pln-blue mx-auto mb-4"
            ></div>
        ...
        <div class="space-y-6" v-if="rack">
            ...
            >
                <div class="px-6 py-4 border-b border-gray-200 bg-light-blue">
                    <h3 class="text-lg font-semibold text-pln-blue">
                        📂 {{ subRack?.name || "Unknown Sub-Rack" }}
                    </h3>
                    <p class="text-sm text-gray-600">
                        {{ subRack?.description || "No description" }}
                    </p>
                    <div class="text-sm text-gray-500 mt-1">
                        {{ getSubRackFiles(subRack?.id).length }} file(s)
                    </div>
                </div>
                                ...
                                <div class="flex items-center">
                                    ...
                                    <div>
                                        ...
                                        <p class="text-xs text-gray-400 mt-1">
                                            Click a viewer button above to
